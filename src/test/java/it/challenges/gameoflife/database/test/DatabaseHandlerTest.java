...
public class DatabaseHandlerTest {
	...
	public void testFindByCycle(){
		...
		Map<Integer,Map<Integer,Cell>> loadedMap = handler.getCycle(4);
		assertNotNull(loadedMap);
		assertTrue(loadedMap.values().parallelStream().flatMap(map->map.values().stream()).allMatch(c->originalCells.get(c.getPosX()).get(c.getPosY()).equals(c)));
	}
